...
def training(X,Price,
    ...
    with tf.Session() as sess:
        ...
        for epc in range(epoch):
            ...
            '''
            earning[epc] = Balance[0] * Price[RecordLength - 1] + Balance[1] - 1e3
            if epc == 0:
                avg[epc] = earning[epc]
            else:
                avg[epc] = avg[epc - 1] + 1.0 / (epc + 1) * (earning[epc] - avg[epc - 1])
    ...
    '''
    plt.figure(figsize = (3, 5), dpi = 120)
    plt.subplot(211)
    plt.plot(earning)
    plt.title('Total earning record')
    plt.subplot(212)
    plt.plot(avg)
    plt.title('Average earning record')
